...
class NERModel(LanguageModel):
    ...
    def load_data(self, debug=False):
        ...
        # Load the starter word vectors 读取词和词对应的向量
        ...
        # x_train 为单词的index窗口, y_train为标签的index
    ...
    def add_embedding(self):
        ...
        (Words are discrete（离散的） entities. They need to be transformed into vectors for use
        ...
        with tf.device('/cpu:0'):
            ...
            window = tf.reshape(window, [-1, self.config.window_size * self.config.embed_size])  # reshape:矩阵变换
            # END YOUR CODE
            return window
    ...
    def add_model(self, window):
        ...
        with tf.variable_scope('Layer1', initializer=xavier_weight_init()) as scope:  # 用initializer=xavier去初始化第一层
            ...
            # matmul 矩阵相乘
            y = tf.matmul(window, W) + b1
            h = tf.nn.tanh(y)  # h是激活函数，双曲正切
            if self.config.l2:  # L2 regularization for W   L2正则化避免过度拟合
        ...
        # 使输入tensor中某些元素变为0，其它没变0的元素变为原来的1/keep_prob大小！ 0.9
    ...
    def add_loss_op(self, y):
        ...
        cross_entropy = tf.reduce_mean(  # 1.关键步骤：loss是用cross entropy定义的
            tf.nn.softmax_cross_entropy_with_logits(logits=y,
                                                    labels=self.labels_placeholder))  # y是模型预测值，计算cross entropy
    ...
    def __init__(self, config):
        ...
        one_hot_prediction = tf.argmax(self.predictions, 1)  # 找到预测的类别的位置， 函数返回最大值的索引
        correct_prediction = tf.equal(
            tf.argmax(self.labels_placeholder, 1), one_hot_prediction)  # 看预测和实际的两个位置是否相等
        self.correct_predictions = tf.reduce_sum(
            tf.cast(correct_prediction, 'int32'))  # 3，关键步骤：将correct转换type，然后求和计算多少预测对了
    ...
    def predict(self, session, X, y=None):
        ...
        for step, (x, y) in enumerate(data):
            ...
            if np.any(y):
                ...
                preds = session.run(self.predictions, feed_dict=feed)  # 1.没有y的时候，喂进去x，执行 predictions
        ...
        with tf.Session() as session:
            ...
            for epoch in range(config.max_epochs):
                ...
                val_loss, predictions = model.predict(session, model.X_dev,
                                                      model.y_dev)  # 2.用这个model去预测 dev 数据，得到loss 和 prediction
                ...
                confusion = calculate_confusion(config, predictions,
                                                model.y_dev)  # 3.把 dev 的lable数据放进去，计算prediction的confusion
