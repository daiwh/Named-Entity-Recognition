    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            inputs = tf.nn.embedding_lookup(embedding,
                                            self.input_placeholder)  # Looks up ids in a list of embedding tensors.
            inputs = [
                tf.squeeze(x, [1]) for x in
                tf.split(1, self.config.num_steps, inputs)]  # remove specific dimensions of size 1 at postion=[1]
            ### END YOUR CODE
            return inputs
    ...
    def run_epoch(self, session, data, train_op=None, verbose=10):
        ...
        for step, (x, y) in enumerate(
            ...
            loss, state, _ = session.run(
                ...
                feed_dict=feed)  # 用RNN的final state，计算loss，并用train训练到最小loss
...
def test_RNNLM():
    ...
    with tf.Session() as session:
        ...
        session.run(init)
        for epoch in range(config.max_epochs):  # 迭代max epoch次
